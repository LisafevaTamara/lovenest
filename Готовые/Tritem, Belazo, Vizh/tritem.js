function tritem(){
    ...
    if(code=="decode"){
        ...
        while(textnew.indexOf('тчк') != -1){
            textnew = textnew.replace('тчк','. ');
        }
        while(textnew.indexOf('зпт') != -1){
            textnew = textnew.replace('зпт',', ');
        }
        while(textnew.indexOf('тр') != -1){
            textnew = textnew.replace('тр','-');
        }
        while(textnew.indexOf('прб') != -1){
            textnew = textnew.replace('прб',' ');
        }
        console.log(textnew);
    }
